...
def game_loop(message):
    global night
    bot.send_message(message.chat.id, 'Добро пожаловать в игру, вам дается 2 минуты чтобы познковаться')
    sleep(120)
    x = 2 
    while True:
        if night:
...
def kick(message):
    ...
    if not night:
        ...
        return
    bot.send_message(message.chat.id, 'Сейчас ночь, нечего голосовать')
