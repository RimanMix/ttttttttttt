...
def players_amount():
    conn = sqlite3.connect('mafia/db.db')
    cursor = conn.cursor()
...
def get_mafia_usernames():
    con = sqlite3.connect('mafia/db.db')
    cursor = con.cursor()
...
def vote(type, username, player_id):
    con = sqlite3.connect('mafia/db.db')
    cursor = con.cursor()
    sql = f'SELECT username From players WHERE players_id = {player_id} and dead = 0 and voted = 0'
    #print(sql)
    cursor.execute(sql)
    ...
    if can_vote:
        ...
        cursor.execute(sql)
        sql = f"UPDATE players SET voted = 1 WHERE players_id = {player_id}"
        cursor.execute(sql)
        con.commit
        con.close
        return True
...
def mafia_kill():
    ...
    cursor.execute(f"SELECT COUNT(*) FROM players WHERE role = 'mafia' and dead = 0")
...
def citizen_kill():
    ...
    citizen_voted = cursor.fetchone()[0]
    cursor.execute(f"SELECT COUNT(*) FROM players WHERE role = 'citizen' and dead = 0")
    citizens = cursor.fetchone()[0]
    killed = 'Никого'
    if citizens == citizen_voted:
        cursor.execute(f"SELECT username From players WHERE citizen_vote = {citizen_voted}")
        killed = cursor.fetchone()[0]
        cursor.execute(f"UPDATE players SET dead = 1 WHERE username = '{killed}'")
        con.commit()
    con.close
